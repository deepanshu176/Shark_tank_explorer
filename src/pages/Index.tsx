...
const Index = () => {
  ...
  const filtered = useMemo(() => {
    return startups.filter((s) => {
      ...
      const matchSeason =
        season === "all" || s.season === Number(season);

      const matchEpisode =
        episode === "all" || s.episode === Number(episode);

      // ✅ SAFE SHARK MATCH (trim + undefined safe)
      const matchShark =
        shark === "all" ||
        (Array.isArray(s.sharks) &&
          s.sharks.map((x) => x.trim()).includes(shark.trim()));
  ...
  const hasActiveFilters =
    search !== "" || season !== "all" || shark !== "all" || deal !== "all";
      ...
      <header className="relative overflow-hidden border-b border-border/30">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 z-10">
          ...
          >
            ...
            >
              Explore all startups that pitched on Shark Tank India. Filter by
              season, shark, or deal status.
            </motion.p>
            ...
            >
              <Badge
                variant="outline"
                className="glass px-5 py-2.5 text-sm font-medium border-border/30"
              >
              ...
              <Badge
                variant="outline"
                className="glass px-5 py-2.5 text-sm font-medium bg-emerald-500/10 text-emerald-400 border-emerald-500/20 shadow-lg shadow-emerald-500/10"
              >
              ...
              <Badge
                variant="outline"
                className="glass px-5 py-2.5 text-sm font-medium bg-rose-500/10 text-rose-400 border-rose-500/20"
              >
        ...
        {filtered.length > 0 ? (
          ...
          >
            ...
            <p className="mt-3 text-muted-foreground max-w-md">
              Try adjusting your filters or search term to discover amazing
              startups
            </p>
          </motion.div>
        )}
